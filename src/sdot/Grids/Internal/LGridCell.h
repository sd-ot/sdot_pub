...
struct LGridCell {
    ...
    union                     DiracData    { LGridCell *next_fcell; Dirac diracs[ 0 ]; };
    union                     Child        { LGridCell *ptr; std::size_t off; };
    union                     Data         { Child children[ 0 ]; DiracData dirac_data; };
    ...
    Dirac&                    dirac        ( int index ) { return data.dirac_data.diracs[ index ]; }
    ...
    Span<Dirac>               diracs       () { return { data.dirac_data.diracs, std::size_t( nb_diracs ) }; }

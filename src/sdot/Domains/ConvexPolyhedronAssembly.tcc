...
template<class Pc>
void ConvexPolyhedronAssembly<Pc>::display_boundaries( VtkOutput &vtk_output ) const {
    for( const Item &item : items )
        item.polyhedron.display( vtk_output, { item.coeff }, false );
}

template<class Pc>
void ConvexPolyhedronAssembly<Pc>::display_coeffs( VtkOutput &vtk_output ) const {
    for( const Item &item : items )
